...
export default function BreakDownOfImprovementCostsDwellings ({ heatData, localAuthority }) {
    ...
    const handleGeneratePDF = () => {
        try{
            graphToPdf('breakDownOfImprovementCostsDwellings',
                `Breakdown of improvement costs by dwellings for ${localAuthority.toString()} - 2018`);
    ...
    useEffect(() => {
        ...
        const dwellingBreakdown = filteredData.reduce((acc, curr) => {
            ...
            ['detached', 'flat', 'semi-detached', 'terraced'].forEach((dwellingType) => {
                ...
                const totalDwellingValue = curr[`Average energy efficiency improvements costs of ${dwellingType} (GBP)`];
        ...
        const formattedData = Object.entries(dwellingBreakdown).map(([dwellingType, value], index) => ({
            id: dwellingType,
            value: parseFloat(value.toFixed(2)), // Convert to a percentage format
